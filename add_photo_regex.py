...
pattern = r'<div style=\{\{\s*background:\s*\'#F8FAFC\',[^<]+<div[^<]+<i data-lucide="camera"[^<]+</i>\s*</div>\s*<div[^<]+No Image Available</div>\s*</div>\s*<button type="button"[^>]+>.*?Capture Photo</button>\s*<button type="button"[^>]+>.*?Upload Document</button>'
