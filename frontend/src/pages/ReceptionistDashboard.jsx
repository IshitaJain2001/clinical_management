              ...
              <div style={{ display: 'flex', flex: 1, minHeight: 0, background: '#FFFFFF' }}>
                ...
                <div style={{ flex: 1, padding: '16px', borderRight: '1px solid #E2E8F0', display: 'flex', flexDirection: 'column', gap: '16px', overflowY: 'auto' }}>
                  ...
                  {isExistingPatient === null && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', background: '#F8FAFC', padding: '10px 16px', borderRadius: '8px', border: '1px solid #E2E8F0' }}>
                      <i data-lucide="search" style={{ width: '16px', color: '#64748B' }}></i>
                      <div style={{ width: '100px', fontSize: '12px', fontWeight: 700, color: '#334155' }}>Search Patient</div>
                      <input type="text" placeholder="Enter Mobile Number or Patient ID..." style={{ width: '280px', height: '32px', fontSize: '12px', padding: '0 12px', border: '1px solid #CBD5E1', borderRadius: '6px', outline: 'none', transition: 'border-color 0.2s' }} onFocus={e => e.target.style.borderColor = '#3B82F6'} onBlur={e => e.target.style.borderColor = '#CBD5E1'} value={searchPatientQuery} onChange={e => setSearchPatientQuery(e.target.value)} />
                      {searchPatientQuery.trim().length > 0 && (
                        <div style={{ position: 'absolute', top: '150px', left: '160px', width: '400px', maxHeight: '200px', overflowY: 'auto', background: 'white', border: '1px solid #E2E8F0', borderRadius: '8px', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)', zIndex: 100 }}>
                          {patientsList.filter(p => { const q = searchPatientQuery.toLowerCase(); return p.name.toLowerCase().includes(q) || p.contact.toLowerCase().includes(q) || p._id.toLowerCase().includes(q); }).length === 0 ? (
                            <div style={{ padding: '12px', fontSize: '12px', cursor: 'pointer', background: '#FEF2F2', color: '#991B1B', fontWeight: 600, textAlign: 'center' }} onClick={() => { setSelectedPatient(null); const isNumeric = /^\d+$/.test(searchPatientQuery.trim()); setFormData({ name: !isNumeric ? searchPatientQuery : '', age: '', gender: '', contact: isNumeric ? searchPatientQuery : '', email: '', doctorId: formData.doctorId, bloodGroup: '', address: '', medicalHistory: '', referredBy: '', allergies: 'None', currentMedications: '' }); setIsExistingPatient(false); setSearchPatientQuery(''); }}>No match found. Click here to register new patient.</div>
                          ) : (
                            patientsList.filter(p => { const q = searchPatientQuery.toLowerCase(); return p.name.toLowerCase().includes(q) || p.contact.toLowerCase().includes(q) || p._id.toLowerCase().includes(q); }).map(p => (
                              <div key={p._id} style={{ padding: '10px 12px', fontSize: '12px', borderBottom: '1px solid #F1F5F9', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }} onClick={() => { setSelectedPatient(p); setFormData({ name: p.name, age: p.age, gender: p.gender, contact: p.contact, email: p.email || '', bloodGroup: p.bloodGroup || 'O+', address: p.address || '', medicalHistory: p.medicalHistory ? p.medicalHistory.join(', ') : '', doctorId: formData.doctorId, allergies: p.allergies || 'None', currentMedications: p.currentMedications || '' }); setIsExistingPatient(true); }}>
                                <div><div style={{ fontWeight: 700, color: '#0F172A' }}>{p.name}</div><div style={{ color: '#64748B', fontSize: '11px', marginTop: '2px' }}>{p.contact}</div></div>
                                <span style={{ color: '#2563EB', fontWeight: 600, background: '#EFF6FF', padding: '4px 8px', borderRadius: '4px' }}>Select</span>
                              </div>
                            ))
                          )}
                        </div>
