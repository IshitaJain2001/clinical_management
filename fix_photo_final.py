...
# 2. Add hidden inputs and replace No Image Available / Buttons
# Find exactly the right sidebar image block
pattern = r'<div style=\{\{\s*border:\s*\'1px dashed #CBD5E1\'[^>]*>.*?No Image Available</span>\s*</div>\s*<button type="button"[^>]*>.*?Capture Photo</button>\s*<button type="button"[^>]*>.*?Upload Document</button>'

new_ui = """<input type="file" id="patientPhotoUpload" style={{ display: 'none' }} accept="image/png, image/jpeg" onChange={(e) => { if (e.target.files && e.target.files[0]) { setPatientPhoto(URL.createObjectURL(e.target.files[0])); } }} />
...
# Use image/png, image/jpeg instead of image/* to completely avoid the syntax issue
...
if re.search(pattern, text, re.DOTALL):
    ...
    print('Failed to find old UI with regex.')
