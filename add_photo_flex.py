...
# Find <div style={{ flex: 1 }}></div> which is right below the buttons.
# We will replace everything from `<div style={{ background: '#F8FAFC', borderRadius: '8px', border: '1px dashed #CBD5E1'` 
# down to `<div style={{ flex: 1 }}></div>`

pattern = r'<div style=\{\{\s*background:\s*\'#F8FAFC\',[^<]*borderRadius:\s*\'8px\'[^{]*\{\{\s*width:\s*\'48px\'[^>]*>.*?No Image Available.*?</div>\s*</div>.*?<div style=\{\{\s*flex:\s*1\s*\}\}></div>'
...
if re.search(pattern, text, re.DOTALL):
    ...
    print('Failed to find old UI with flex pattern.')
